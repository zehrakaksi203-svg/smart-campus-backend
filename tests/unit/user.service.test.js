...
describe("user.service (unit)", () => {
  ...
  describe("getMe", () => {
    test("kullanıcı bulunduğunda şifre hariç kullanıcıyı döner", async () => {
      ...
      expect(result).toEqual({ id: 1, fullName: "Test", email: "t@test.com" });
    });
    ...
    test("rol Student ise studentProfile alanı eklenir", async () => {
      const fakeUser = {
        id: 1,
        role: "Student",
        toJSON: jest.fn().mockReturnValue({ id: 1, role: "Student" })
      };
      User.findByPk.mockResolvedValue(fakeUser);
      Student.findOne.mockResolvedValue({ id: 10, studentNumber: "S-1" });
      ...
      expect(result.studentProfile).toMatchObject({ id: 10 });
      expect(result.facultyProfile).toBeUndefined();
    });

    test("rol Faculty ise facultyProfile alanı eklenir", async () => {
      const fakeUser = {
        id: 2,
        role: "Faculty",
        toJSON: jest.fn().mockReturnValue({ id: 2, role: "Faculty" })
      };
      User.findByPk.mockResolvedValue(fakeUser);
      Faculty.findOne.mockResolvedValue({ id: 20, employeeNumber: "F-1" });

      const result = await userService.getMe(2);

      expect(Faculty.findOne).toHaveBeenCalledWith(
        expect.objectContaining({ where: { userId: 2 } })
      );
      expect(result.facultyProfile).toMatchObject({ id: 20 });
